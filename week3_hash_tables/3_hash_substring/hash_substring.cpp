...
long long get_hash(const string &s, const long long &p, const long long &x) {
  long long hash{0};
...
std::vector<long long> rabin_karp(const string &text,const size_t &psize, const long long &p, const long long &x) {
  size_t size = text.size() - psize + 1;
  std::vector<long long> hasht(size);

  auto begin = text.begin()+size-1;
  auto end = text.end();
  string last(begin, end);
  hasht[size - 1] = get_hash(last, p,x);
 ...
 for (int k = (int)size - 1; k > 0; --k) {
   hasht[k - 1] = (x * hasht[k] + text[k - 1] - text[k + psize - 1] * x_to_psize) % p;
  //  Normalize mod p value to positive representative.
   hasht[k - 1] = (hasht[k - 1] + p) % p;
}
  return hasht;
}
...
std::vector<int> get_occurrences(const Data& input) {
  ...
  string t = input.text;
  string pa = input.pattern;

  int psize = pa.size();
  long long phash = get_hash(pa,p,x);
  auto hasht = rabin_karp(t,psize,p,x);

  std::vector<int> indices;
  
  for(int i = 0;i<hasht.size();i++) {
    if(hasht[i] == phash) {
      auto first = input.text.begin() + i;
      auto last = input.text.begin() +i+psize;
      string check(first,last);
